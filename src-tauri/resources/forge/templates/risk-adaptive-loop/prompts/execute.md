...
3. Update `plans/{{plan_id}}/state.json`:
   ...
   - If any check is unmet (including unresolved critical findings in review), keep this phase non-completed as `blocked` or `failed` so execution stops until fixed.
...
4. Forge will run backend checks after your phase is marked complete.
   ...
   - In `review-gate`, if any unresolved critical finding remains, keep the phase non-completed (`blocked` or `failed`) and retry after fixes.
